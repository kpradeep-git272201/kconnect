...
@Component({
  selector: 'app-see-classwork',
  templateUrl: './see-classwork.page.html',
  styleUrls: ['./see-classwork.page.scss'],
  standalone: true,
  imports: [SharedModule]
})
export class SeeClassworkPage implements OnInit {
  classList: any=[];
  subjectList: any=[];
  months: any=[
  ...
  async viewWork(selectedClass,selectedSubject,selectedMonth){
    ...
    if(selectedClass!=null && selectedSubject!=null && selectedMonth!=null){
      await this.loadingService.showLoading();  
      this.commonService.getSeeClasswork(this.selectedMonth, this.selectedSubject, this.selectedClass, 0).subscribe(async (resp)=>{
        if(resp?.body.length>0){
          await this.loadingService.hideLoading();
